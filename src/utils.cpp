...
std::vector<sf::Vector2f> coord_CP(int nbCP){
    ...
    for (int i=0; i<nbCP+1; ++i)
    {
        ...
        float y;
        do{
            //Definition aleatoire des coordonnes du checkpoint
            x = rand()%14801 + 600; // x dans [600,15400] pour que le checkpoint ne sorte pas de la fenetre
            y = rand()%7801 + 600; // y dans [600, 8400]
            collision = false;
            
            // Verification d'une eventuelle collision avec les checkpoints precedents
            for(int j=0; j<i; ++j){
        ...
        //Si pas de collision, on ajoute les coordonnees du checkpoints
...
{
    sf::Vector2f C = target-pos; //On construit le vecteur qui part du pod vers la target
...
{
    /*
    La puissance du pod sera maximale si le prochain checkpoint est devant lui.
    Cependant, si il est derriere, le pod a besoin de faire demi-tour. Pour cela il diminue sa puissance pour eviter de faire un trop grand demi-tour.
    */
